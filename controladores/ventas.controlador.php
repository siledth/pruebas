	...
	static public function ctrCrearVenta(){
		...
		if(isset($_POST["nuevaVenta"])){
			...
			foreach ($listaProductos as $key => $value) {
			  ...
			  //ACTUALIZA STOCK
				...
				if ($valor1b<0  && $_POST["TipoVenta"]=="VEN"){
						...
						swal({
							  type: "error",
							  title: "La cantidad que se quiere  es superior al stock",
							  showConfirmButton: true,
							  confirmButtonText: "Cerrar"
							  }).then(function(result){
										if (result.value) {
			...
			if($contadorProducto==0){
				...
				swal({
					  type: "error",
					  title: "La programación no se ha ejecuta si no hay productos",
					  showConfirmButton: true,
					  confirmButtonText: "Cerrar"
					  }).then(function(result){
								if (result.value) {
			...
			$datos = array("id_usuario"=>$_POST["idVendedor"],
			                "id_entes"=>$_POST["idVendedor"],
						   "id_accion_centralizada"=>$_POST["seleccionarCliente"],
						   "id_partida_presupuestaria"=>$_POST["seleccionarpartida"],
						   "id_fuente_financiamiento"=>$_POST["seleccionarfuentefinanciamiento"],
						   "id_actvcomercial"=>$_POST["seleccionaractividad"],
						   "id_municipio"=>$_POST["seleccionarmunicipio"],
						   "id_estado"=>$_POST["seleccionarestado"],
						   "trimestre"=>$_POST["seleccionartrimestre"],
						   "codigo_onapre"=>$_POST["onapre"],
						   "codigo_anual"=>$_POST["nuevaVenta"],
						   "id_ccnu"=>$_POST["listaProductos"],
						   "alicuota_iva"=>$_POST["nuevoPrecioImpuesto"],
						   "monto_iva"=>$_POST["nuevoPrecioImpuesto"],
						   "fecha"=>$_POST["fecha"],
						   "costo_unitario"=>$_POST["nuevoPrecioNeto"],
						   "precio_total"=>$_POST["totalVenta"],
						   "monto_total"=>$_POST["totalVenta"],
						   "ano"=>$_POST["TipoVenta"],
						   "mes"=>$_POST["TipoVenta"],
						   "fecha"=>$_POST["fecha"],
						   "monto_partida"=>$_POST["Observaciones"],
						   "monto_partida2"=>$_POST["Observaciones"],
						   "monto_partida3"=>$_POST["Observaciones"],
						   "name_proyecto"=>$_POST["cotizarA"]);
	     ...
	     // exit($datos);
			...
			$datosBitacora["descripcion"]="SE GUARDO : ". $_POST["nuevaVenta"];
			...
			$respuestaBitacora = ModeloBitacora::mdlIngresarBitacora("bitacora", $datosBitacora);
				...
				if ($_POST["TipoVenta"]=="COT"){
					...
					$strMensaje="La Programación ha sido guardada correctamente";
